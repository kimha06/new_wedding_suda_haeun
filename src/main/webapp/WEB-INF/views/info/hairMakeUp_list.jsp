	...
	<div id="contain02_text">
	    ...
	    <!-- <form name="hall_name" id="hall_name2" method="post"> -->
			<div class="input_wrap">
	            <form name="hall_name" id="hall_name2" method="post" action="./hairMakeUp_list">
	...
	<c:forEach var="hMDto" items="${map.list }" >
		<div class="SHOP_one_bx">
			...
			<div class="SHOP_text_wrap">
				<span class="SHOP_text_name" style="color:#000000;">${hMDto.com_name }</span>
...
<div id="board_link_wrap">
		<div id="board_link_num">
			<c:choose>
				...
				<c:otherwise>
            		<a href="hairMakeUp_list?search=${map.search }&page=1" title="1페이지">&lt;&lt;</a>
				</c:otherwise>
			</c:choose>
			<c:choose>
				<c:when test="${map.page <= 1 }">
				...
				<c:otherwise>
            		<a href="hairMakeUp_list?search=${map.search }&page=${map.page-1}" title="이전">&lt;</a>
				</c:otherwise>
			</c:choose>
			<!-- 페이지 번호 반복 -->
			<c:forEach var="nowPage" begin="${map.startPage}" end="${map.endPage }">
				<c:choose>
			          ...
			          <c:otherwise>
			             <a href="hairMakeUp_list?search=${map.search }&page=${nowpage}" style="font-size:16px;">${nowPage }</a>
			          </c:otherwise>
			        </c:choose>
			</c:forEach>
			<c:choose>
				...
				<c:otherwise>
					<a href="hairMakeUp_list?search=${map.search }&page=${map.page+1}" title="다음" class="num_right">&gt;</a>
				</c:otherwise>
			</c:choose>
			<c:choose>
				<c:when test="${map.page >= map.maxPage }">
				...
				<c:otherwise>
					<a href="hairMakeUp_list?search=${map.search }&page=${map.maxPage}" title="끝페이지" class="num_right">&gt;&gt;</a>
				</c:otherwise>
			</c:choose>
        </div>
